...
# FILENAME TO SEQAN LOG FILES:
openms_log_file <- "../logfiles/openms.logs"
...
global_logdata <- read.table(openms_log_file, sep = "\t", 
                             fill = T, header = F, quote = "", 
                             stringsAsFactors = F)
colnames(global_logdata) <- c("calls", "date", "ip", "app", "version", "cpu", 
                              "os", "who", "clustered", "country", "region", 
...
new_unique_ips_per_month <- function(data)
{
  ...
  barplot(plot_data$New_Unique_Users, col = "lightblue",
          ...
          las = 2,
          main = "New (unique) OpenMS User Count per Month")
  par(new=T)
...
total_ips_per_month <- function(data)
{
  ...
  barplot(plot_data$Total_Usage, col = "lightblue",
          ...
          las = 2,
          main = "OpenMS Usage Count per Month")
  par(new=T)
