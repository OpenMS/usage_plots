...
source('global_openms.R')

```

This is an automatically generated report that evaluates the current OpenMS user statistics. The data is taken from the file `../logfiles/openms.logs` preprocessed by a script `global_openms.R`. If you want to evaluate this report on another file, please change the variable `openms_log_file` in the script `global_openms.R`.
...
## The Data
The collected infromation contains the following columns:
...
desc <- c("The number of calls.",
          ...
          "A Parameter only used in OpenMS. (Here: OpenMS)",
...
knitr::kable(cbind(1:length(names) , names, desc), caption = "The column names of the collected user information.")
```

Some preprocessing details (`global_openms.R`):

* All entries marked as clusters are removed from the data set.
* A seperate table keeps track of the amount of cluster uses.
...
To display the general usage of Seqan, the plot shows the cumulative usage of all application summed together over the given time period.
...
plot(plot_data[,1], cumsum(plot_data[,2]), type = "l",xaxt="n",
     ...
     main="Cumulative Sum of All Calls of OpenMS")
...
polygon(c(plot_data[,1], rev(plot_data[,1])),
        ...
        col='lightblue')
dev.copy(png,  paste("figures_and_tables/OpenMS_total_cumulative_sum_of_OpenMS_usages_",
                     MAX_DATE, ".png", sep = ""))
dev.off()
```
...
## Distribution of Countries and Cities per OpenMS Application
...
write.table(t, sep = "\t",
            file = paste("figures_and_tables/OpenMS_Country_and_city_distribution_per_app_",
                         MAX_DATE, ".tsv", sep = ""))
...
## Daily and Weekly usage of OpenMS Applications
...
write.table(t, sep = "\t",
            file = paste("figures_and_tables/OpenMS_Daily_and_weekly_usage_per_app_",
                         MAX_DATE, ".tsv", sep = ""))
...
dev.copy(png,  paste("figures_and_tables/OpenMS_top10_applications_by_calls_",
                     MAX_DATE, ".png", sep = ""))
dev.off()
```
...
dev.copy(png,  paste("figures_and_tables/OpenMS_top10_applications_by_users_",
                     MAX_DATE, ".png", sep = ""))
dev.off()
```
...
dev.copy(png,  paste("figures_and_tables/OpenMS_operating_system_distribution_",
                     MAX_DATE, ".png", sep = ""))
dev.off()
```
...
dev.copy(png,  paste("figures_and_tables/OpenMS_unique_new_users_per_month_",
                     MAX_DATE, ".png", sep = ""))
dev.off()
```
...
```{r echo=F, fig.height=4, results='hide'}
total_ips_per_month()
dev.copy(png,  paste("figures_and_tables/OpenMS_total_calls_per_month_",
                     MAX_DATE, ".png", sep = ""))
dev.off()
```
