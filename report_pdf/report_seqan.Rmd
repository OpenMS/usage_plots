...
knitr::opts_chunk$set(echo = FALSE, 
                      ...
                      warning = F,
                      fig.align = "center")
source('global_seqan.R')

```

This is an automatically generated report that evaluates the current SeqAn user statistics. The data is taken from the file `../logfiles/seqan.logs` preprocessed by a script `global_seqan.R`. If you want to evaluate this report on another file, please change the variable `seqan_log_file` in the script `global_seqan.R`.
...
Some preprocessing details (`global_seqan.R`):
...
plot(plot_data[,1], cumsum(plot_data[,2]), type = "l",xaxt="n",
     ...
     main="Cumulative Sum of All Calls of SeqAn")
...
polygon(c(plot_data[,1], rev(plot_data[,1])),
        ...
        col='lightblue')
dev.copy(png, paste("figures_and_tables/SeqAn_total_cumulative_sum_of_SeqAn_usages_",
                     MAX_DATE, ".png", sep = ""))
dev.off()
```
...
## Distribution of Countries and Cities per SeqAn Application
...
write.table(t, sep = "\t",
            file = paste("figures_and_tables/SeqAn_Country_and_city_distribution_per_app_",
                         MAX_DATE, ".tsv", sep = ""))
...
## Daily and Weekly usage of SeqAn Applications
...
write.table(t, sep = "\t",
            file = paste("figures_and_tables/SeqAn_Daily_and_weekly_usage_per_app_",
                         MAX_DATE, ".tsv", sep = ""))
...
dev.copy(png, paste("figures_and_tables/SeqAn_top10_applications_by_calls_",
                     MAX_DATE, ".png", sep = ""))
dev.off()
```
...
dev.copy(png, paste("figures_and_tables/SeqAn_top10_applications_by_users_",
                     MAX_DATE, ".png", sep = ""))
dev.off()
```
...
dev.copy(png,  paste("figures_and_tables/SeqAn_operating_system_distribution_",
                     MAX_DATE, ".png", sep = ""))
dev.off()
```
...
dev.copy(png,  paste("figures_and_tables/SeqAn_unique_new_users_per_month_",
                     MAX_DATE, ".png", sep = ""))
dev.off()
```
...
```{r echo=F, fig.height=4, results='hide'}
total_ips_per_month()
dev.copy(png,  paste("figures_and_tables/SeqAn_total_calls_per_month_",
                     MAX_DATE, ".png", sep = ""))
dev.off()
```
